...
> **实现状态注释（2026-04-16）**：当前代码中仅 `rules` 和 `lessons` 按 scope 过滤。`techStack`、`archSummary`、`avoid`、`domainModel` 对所有 Agent 全量注入。Phase 1 计划对齐下方矩阵。详见 `architecture-decision-records.md` ADR-006。
...
- [x] 更新 `ProjectContext` 数据模型（TypeScript + Python）— ✅ 已完成
- [x] 新建 `backend/app/context.py`：`AGENT_SCOPE_MAP` + `assemble_context()` — ✅ 已完成
- [x] Worker Agent 执行时接入 `assemble_context()` — ✅ 已完成（`workflow_engine.py` → `execute_worker_agent`）
- [x] 更新 `ProjectSettingsModal` 适配新字段 UI — ✅ 已完成
- [x] 更新 AI 生成上下文接口，生成新结构字段 — ✅ 已完成

### P1 — 下一个 Sprint（沉淀闭环）

- [ ] 路径 A：步骤完成后的"修改捕获弹窗" — **未开始**
- [x] 路径 B 后端：Agent 输出增加 `suggestedContextUpdates` 字段 — ✅ 已实现
- [x] 路径 B 后端：`pending_suggestions` 表 + 确认/拒绝 API — ✅ 已实现
- [ ] 路径 B 前端：待确认的上下文建议列表 UI — **未开始**
- [ ] Winston 专项：架构决策结构化捕获（含被否方案） — **未开始**
- [ ] 上下文注入矩阵完整对齐（per-agent 字段过滤） — **未开始**（见 ADR-006）
...
*v0.3 — 2026-04-16 — 与代码实现对齐：P0 已全部完成，Path B 后端已实现，Commander scope 已添加*
